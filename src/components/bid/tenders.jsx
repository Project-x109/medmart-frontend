...
const Tenders = () => {
    ...
    const [downloadsToggle, setDownloadsToggle] = useState(true);
    ...
    return (
        <>
            <MetaData title="All Products | Medmart" />
                            ...
                            <div className="flex flex-col gap-2 py-3 text-sm overflow-hidden">
                                ...
                                <div className="flex flex-col gap-2 border-b px-4">
                                    ...
                                    <div className="flex gap-3 items-center justify-between mb-2 min-w-full">
                                        <span className="flex-1 border px-4 py-1 rounded-sm text-gray-800 bg-gray-50">{price[0].toLocaleString()} Birr</span>
                                        <span className="font-medium text-gray-400">to</span>
                                        <span className="flex-1 border px-4 py-1 rounded-sm text-gray-800 bg-gray-50">{price[1].toLocaleString()}Birr</span>
                                    </div>
                                </div>
                                {/* price slider filter */}
...
<div className="flex justify-between cursor-pointer py-2 pb-4 items-center" onClick={() => setDownloadsToggle(!downloadsToggle)}>
    <p className="font-medium text-xs uppercase">Downloads</p>
    {downloadsToggle ?
        <ExpandLessIcon sx={{ fontSize: "20px" }} /> :
        <ExpandMoreIcon sx={{ fontSize: "20px" }} />
    }
...
{downloadsToggle && (
    <div className="flex flex-col pb-1">
        <FormControl>
            ...
            >
                {[4, 3, 2, 1,0].map((el, i) => (
                    <FormControlLabel value={el} key={i} control={<Radio size="small" />} label={<span className="flex items-center text-sm">{el*100}  & above</span>} />
                ))}
            </RadioGroup>
        </FormControl>
    </div>
